...
- [x] **Task 1 — enumerate the clause set mechanically, not by reading** (AC: 3)
  ...
  - [x] The chapter boundary is clean, and this was verified rather than assumed: `grep -rl 'tck-id-payloads-'` over `docs/spec/sparkplug-b-3.0.0/chapters/` returns **only** `Sparkplug_6_Payloads.adoc`, and the same pattern over the whole pinned tree yields the same **109** ids. No `payloads-*` clause hides in another chapter, and chapter 6 holds nothing else. Re-run both if you doubt the worklist.
...
Found by reading the norm rather than the code, which is the whole argument for the pinned specification. Do not let it dissolve into a generic "datatype clauses: conformant" row.
...
| Check | Method | Result |
| --- | --- | --- |
| Clause enumeration | `grep` over `Sparkplug_6_Payloads.adoc` | 109 ids; boundary re-verified — only that file carries `payloads-*`, and the whole pinned tree yields the same 109 |
...
| Spec claims | read the pinned norm directly | both editorial defects, NCMD/DCMD wording, bdSeq increment, `-disconnect-mqtt311` conditional, quality codes — all confirmed verbatim |
