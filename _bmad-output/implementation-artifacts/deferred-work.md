...
- ~~String-key semantics before Epic 2 validation: `Serial::new("")` keys collide and `Eq`/`Hash` are case-sensitive (API vs config casing drift would create two logical devices) while Stories 1.9/1.10 already key maps on `Serial`/`MeterId`. Validation is scoped to Epic 2/5 per the story spec — revisit no later than Story 1.9.~~
  **DELIVERED across 5.1 and ADR 0029** (recorded at Epic 2 close, 2026-08-15): `check_serial`
  validates every configured serial against the topic grammar — an empty string is not a topic
  level, so it is refused at load — and the online identity check refuses at fetch time any
  serial that is not the one smart-me reports, casing drift included. Real serials are digit
  strings, so case-sensitivity has no remaining subject.
- ~~`TopicPath` accepts strings that are invalid as MQTT publish topics (empty string, `+`/`#` wildcards, interior NUL, leading/trailing `/`); failure surfaces only at the broker, far from the construction site. Well-formedness lands inside `new()` in Epic 2/5.~~
  **DELIVERED in substance by the sparkplug-b topic grammar** (story 4.1's audit): construction
  refuses the wildcard/empty shapes at the site. The residual is exactly [#34] — MQTT's
  character-set rule (interior NUL) is not enforced — and that issue carries it.
- Range policy for physical/timestamp values — **swept at Epic 2 close, 2026-08-15, into its
  parts**: `±inf`/NaN → delivered (story 2.5, `value-not-finite`/`value-overflowed`); pre-1970
  `UtcMillis` → delivered (the plausibility floor, both host and cloud sides, story 1.5);
  negative `Kwh` as a *plausibility* judgement → withdrawn with FR14 (ADR 0033: not the
  bridge's to judge; a backwards counter IS caught, as a fact between two readings, story 2.2).
  **Still standing, re-parked on story 4.19** (payload clauses): the `i64→u64` conversion at
  the Sparkplug encode boundary — wire timestamps are `uint64` and a pre-1970 stamp cannot be
  encoded honestly; today the floor makes it unreachable, and the encode boundary should say so
  itself.
...
- ~~Frozen/replayed-feed oracle: a byte-identical replayed response (`http_date` frozen together
  ...
  already carries the inputs" additive oracle the architecture defers to Epic 2.~~
  **DELIVERED 2026-08-13/15, story 2.7** — `feed-not-advancing` (contract 9), recorded on every
  successful fetch, adoption gated on it ([#69]). **One clause of this item is NOT delivered and
  is re-parked here on its own**: the future-dated coherent pair (both stamps shifted forward,
  small age) advances `http_date` between ticks and passes the feed oracle; what would catch it
  is comparing the cloud stamp against the HOST clock, which ADR 0004 keeps out of the freshness
  formula on purpose. Nothing produces this today; it needs its own decision if it ever does.
- ~~`Date`-header 1-second truncation tolerance: a genuinely fresh reading can compute age ≈ −900 ms
  ...
  **CLOSED 2026-08-15 at epic close, by the epic's own standing rule rather than by a band**: a
  tolerance value would be a number nobody measured, which this epic refused three times (story
  2.2 AC4, ADR 0033, story 2.7 AC2). The spurious-STALE direction is fail-safe and no flapping
  has been observed in six days of real polling on panoramix. Reopens the day real polling data
  shows the flapping this predicted.
...
- ~~429 `Retry-After` honoring + rate-limit backoff — Epic 2 (architecture: "rate-limit/429
  backoff + token-refresh handling").~~ **DELIVERED, story 2.6** (the one honoured wait,
  `RETRY_AFTER_CAP`, no general source backoff — a decision, not an omission) and hardened by
  [#76] (the wait ends, the cap caps — both closed 2026-08-15). Residuals carried by [#75]
  (latched cause depends on the last tick) and [#77] (the token endpoint's 429 arms no wait).
- Tolerant per-device list deserialization (today one degraded device fails the whole
  `GET /Devices` array parse; single-device path unaffected) — re-parked 2026-08-15 on
  **story 3.4**, which is the first consumer of `GET /Devices` (discovery). Epic 2 never touched
  the list path.
...
- ~~`map_device` diagnostics: the three failure modes (unknown unit / non-finite value /
  ...
  **DELIVERED, stories 2.5 and 2.6** — per-field causes (`unit-not-recognised`,
  `value-not-finite`, `value-overflowed`, `source-timestamp-unparseable`, contract 7), and the
  reason reaches an operator (2.6 AC5). The decode-failure-names-no-field residual was [#73],
  closed 2026-08-13.
- ~~`Bad` carrier value `0.0` on a cumulative kWh counter: a consumer that ignores the quality flag
  ...
  omit the metric entirely.~~ **DELIVERED, story 2.5** — `BAD_CARRIER` is removed entirely:
  absence is `None` and a refused metric publishes null, never a substituted number. The
  "omit the metric" option won.
...
- Report-by-exception / duplicate suppression: an unchanged `value_date` republishes the same
  point every poll (duplicate historian points at one timestamp). ~~Epic 2.~~ Carried by
  [#32] since 2026-08-03 (RBE is blocked on Rebirth); noted at Epic 2 close that the epic
  deliberately did NOT take it — ADR 0027's every-cycle republish is the opposite decision,
  made for honesty over historian tidiness, and [#32] is where the tension is recorded.
...
- **`MeterUpdate::meter` is read by no production code.** **[#66]**, item 3 (opened
  ...
  *what answered*, which is the comparison ADR 0029 now performs one layer down. Either it
  earns a reader or it should go. Re-anchored at Epic 2 close (2026-08-15): the epic ended
  without giving it a reader, so the decision rides with [#66] item 3 alone.
...
- **"No opinion" and "I checked and it is fine" are the same value.** On a failed fetch the
  ...
  **DECIDED AT EPIC 2 CLOSE (2026-08-15), B4's third category — not doing it, written.** The
  epic finished its oracle set and nothing came to need the distinction: worst-wins held for all
  seven oracles and no confirmation/coverage rule exists anywhere. An abstain variant with no
  consumer would be the unfalsifiable-mechanism shape this epic spent itself removing
  (`latches()`' history). It reopens with its first consumer, which is a design input that
