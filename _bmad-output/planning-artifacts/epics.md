...
- AR7: **Broker-down policy = traced-drop (no buffer).** `try_publish` non-blocking; on full/broker-down → per-device traced drop: **counted per meter and per reason, traced at WARN with the reading's source timestamp, and readable by an operator without reading the log**. No persistent buffer. *(Amended by [ADR 0036] on 2026-08-18: this named `readings_dropped_total{meter,reason}`, a Prometheus metric with no consumer in this deployment. The property is what binds; `/healthz`'s `dropped_readings` satisfies it today, an exporter remains legitimate and unowned, and AR7 governs the **hand-over**, not the journey — the counts are a floor on what was lost, never a ceiling.)* Anti-replay invariant: every published Sparkplug timestamp == its source `ValueDate`, verified at the down→up reconnection instant.
...
| 2.5 | payload completeness and numeric domain (FR16) | `done` (2026-08-12) — carried ADR 0031 to the source boundary and, after review, to the two memories; **implements no bounds**, FR16's `min/max` clause being what ADR 0033 withdrew. `CONTRACT_VERSION` 6 → 7, additive |
| 2.6 | error taxonomy and bounded backoff (FR4, FR5, NFR1) | `ready-for-dev` (2026-08-12) — splits `source-refused` into three; **builds no general backoff**, the poll period being the retry, and honours `Retry-After` instead |
| 2.7 | UTC end-to-end and clock skew (FR10) | `ready-for-dev` (2026-08-12) — carries the replay oracle parked since Epic 1; **verifies** UTC rather than rebuilding it; should close [#69](https://github.com/guycorbaz/smartme_mqtt/issues/69) |
...
**FR14 was withdrawn on 2026-08-12, and story 2.4 with it** ([ADR 0033](../../docs/adr/0033-fr14-is-withdrawn-physical-plausibility-is-not-the-bridge-s-to-judge.md), [#72](https://github.com/guycorbaz/smartme_mqtt/issues/72)). Physical plausibility was the only judgement in the list requiring knowledge the bridge does not receive and cannot verify — what the supply behind a meter can deliver. The internal contradictions stay, and the test that separates them is whether the bridge can be wrong in a way it cannot detect: a counter going backwards is a fact about two numbers it holds; a power reading "too high" is an opinion about a building. **It cost nothing on the wire** — FR14 had no code, `CONTRACT_VERSION` stays at 6 — and it was the draft of story 2.4, refusing to invent its own bounds, that exposed it. **Consequence carried on [#69], and settled the same day**: the withdrawn story was its only identified subject, so the question became whether a rule no input can distinguish from the one it replaced deserves to stay. **Guy decided it stays** — it is correct by construction while the guard it replaced asks the SOURCE whether a reading is trustworthy, which is the question the oracle layer exists to answer instead. AC3 is therefore unmet by ACCEPTANCE rather than pending anything, and [#69] stays open as its standing record until an oracle first refuses a reading the source called good.
...
**Story 2.3 closed on 2026-08-12, after its FIX COMMIT was reviewed** — the step it had not had. `90a7437` repaired what the review of the implementation found, and a repair is not reviewed by the review that caused it. That pass found three more, one of them live on the surface AC6 exists for: `/` printed *"One meter is not being read: cellar … a restart is needed to clear"* directly above *"One meter is being read … Nothing here is cleared by a restart"*, because `FleetState::degraded` filtered on the published quality alone while `failed` filters on `State::Failed` and `record` writes both on every tick. The operator is not lied to — they are told two opposite things and left to choose, at the hour when they are least able to. Fixed in `47e2d49`, falsified first. **Each pass found less than the one before, which is the only evidence available that this converges.**
...
**Status 2026-08-08:** opened 2026-08-06 out of ADR 0025's forward order (6 → 3 → 2 → 7 → 8), because Guy's gate for the panoramix deployment test — *"the next tests on panoramix when the web UI works with all the meters"* — lands here and nowhere else. **3.1 and 3.2 are implemented**, both in `review`: `RUNTIME_METER_LIMIT` is gone and every enabled meter is served by its own poll task and heartbeat (3.1), and a meter that answers and then stops is republished with the oracle's verdict and its own `ValueDate` rather than withheld (3.2). **AR6's per-meter snapshot is met since 2026-08-08 (story 3.3), after story 3.1 ticked a box for work that did not exist** — see the caveat below. NFR2 and FR12 are not met either: per-meter staleness isolation and its latency bound are story 3.3, and nothing yet measures the bound. FR2, FR6 and FR21 are untouched (3.4–3.6).

> **AR6's `watch<[MeterState; N]>` does not exist, found 2026-08-08.** Story 3.1's task list carries `[x] watch<[MeterState; N]> per AR6, so the UI reads a coherent snapshot rather than N values that never agreed at any instant` — and `tokio::sync::watch` appears nowhere in the crate. What shipped is `poll_publish::Heartbeats`: an `Arc<Vec<(MeterId, LastLoopTick, Arc<AtomicU8>)>>`, read one meter at a time under `Ordering::Relaxed`. It is per-meter, which is the half that mattered for the fleet, and it is **not a snapshot** — a reader walking four meters can observe four different instants, which is the precise failure the ticked line says it prevents. Nothing is known to be wrong on the screen today: `/` and `/healthz` render one page per request and no assertion yet depends on two meters agreeing about *when*. The debt is that the guarantee is claimed and absent, and the day a screen compares meters it will be absent quietly. Owed to Epic 3 with 3.3, where per-meter isolation is the subject and a coherent read is what a latency bound is measured against. **Built there the same day**: `watch<FleetState>` with a generation counter incremented inside every `send_modify`, so a snapshot's total belongs to one instant. The old field-by-field read tears on the 57th write out of 8000 under `the_fleet_is_read_at_one_instant` — it was not unlikely to be torn, it was torn almost immediately whenever anything wrote.

> **The republish came into this epic from Epic 2, and the move is a decision rather than a drift** ([ADR 0027](../../docs/adr/0027-a-failed-source-is-a-fault-the-screen-must-name.md), 2026-08-06). ADR 0027 set the rule — every poll cycle publishes a verdict for every enabled meter, or a device certificate; never silence — and left the mechanism with Epic 2, which has no story. At one meter a withheld verdict is a bridge that looks healthy and says nothing; at four it is worse in kind, because three silences hide behind the one meter that works and one of Guy's four is physically unplugged, so the silent case is the permanent case here. ADR 0025 warned that deferring to a deferred epic is how an item stops being tracked; story 3.2 owns it instead.
>
> **A silent meter gets a non-good quality on DDATA, not a DDEATH.** The norm allows the DDEATH (`Sparkplug_4_Topics.adoc:460`), and [ADR 0012](../../docs/adr/0012-quality-codes-spec-versus-host.md) chose `Bad_Stale` for this exact case, verified against a live Ignition across a `Good`→`Stale` transition on 2026-08-03. A DDEATH additionally destroys the device's *online* state in the host — a heavier claim than "this reading is old", and one the next good poll does not undo. DDEATH stays reserved for disable and for disappearance-from-discovery (3.5).
>
> **Discovery (FR2) is deliberately NOT a prerequisite for the fleet.** The mapping is typed or pasted today and that works; discovery makes it pleasant and makes FR6 possible. Sequencing it first would have put an API-shaped story between Guy and the thing he asked for.
>
> **That deferral now has an observed cost, and story 3.4 gains a second purpose** (2026-08-13). Story 2.6's review found that an unknown device id was published as a network fault and polled for ever; the repair makes it a latching configuration refusal. Guy's reading of it: *"quand on ajoute des compteurs à smartme_mqtt, ils proviennent de ce qui existe chez smart-me : donc un compteur inexistant ne devrait pas apparaître sauf faute de frappe"* — which is exactly the argument for **a drop-down of the account's meters on the configuration screen** rather than a typed id. So 3.4 is not only what makes FR6 possible; it is what makes the fault rare. The data source is already audited: `types.rs` records `GET /Devices` from story 1.1's capture, and only `GET /Devices/{id}` is implemented.
>
> **Prevention and honesty are both required, and the order of priority is honesty first** (decided 2026-08-13, on the same reading). A drop-down cannot replace the refusal: the configuration file is editable by hand so the screen is not the only way in, a device chosen from a valid list can disappear afterwards, and the list is populated at one instant while the fetch happens every period. Making a fault rare does not license reporting it as one that passes by itself. One boundary to watch when 3.4 is written: the screen must call smart-me *before* the mapping is confirmed, and story 5.3 is named "nothing is published until the mapping is confirmed" — not a contradiction, but a line to draw deliberately.
...
**There is no secret field.** *(Was: "a secret field renders empty, always". [ADR 0023](../../docs/adr/0023-the-file-is-the-configuration-the-credential-stays-in-the-environment.md) removed the credential from the UI entirely — the strongest form of never-rendered is never-present. The screens must not reintroduce one.)*
...
*Two things this story does NOT do, stated so they are not read into it. **AR7's `readings_dropped_total{meter,reason}` is not implemented** — there is no metrics registry in this repository and introducing one is Epic 6/7 work. Settled by [ADR 0036] on 2026-08-18 ([#89]): AR7 keeps its intent and loses its mechanism, `/healthz` satisfies it, and an exporter stays legitimate and unowned. And **AC2's file-descriptor half is discharged by construction, not by measurement**: the drop path opens no descriptor. The sustained-load RSS/FD run is Story 4.15's (`AC-LEAK-01`) and is not claimed here.*
