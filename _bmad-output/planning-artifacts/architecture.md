...
Shared per-meter `Fresh|Stale|Failed` state via a **`watch` snapshot** (UI reads a coherent atomic snapshot). The state machine is a **pure module** `(prev, tick, now) → next`, testable without tokio; input is a fixed `tick` struct `{value, value_date, http_date, now}`. ~~Config is **restart-only** (YAGNI hot-reload).~~ **REVERSED 2026-08-03 by FR46** ([ADR 0021](../../docs/adr/0021-configuration-is-editable-from-the-ui.md)): configuration is editable from the UI *"without editing a file or restarting the container"*, so hot reload is no longer YAGNI — it is the requirement. AR8's `ArcSwap<Config>` is what carries it, and line 426 already assumed it while this sentence denied it. **Not every field is hot**: changing the group or node identity changes the topic namespace and the will registered at CONNECT, which is a new session by definition. Story 5.2 owes a per-field table — hot vs requires-reconnect — with **no field left undecided**, because an unlisted field will be assumed hot by whoever writes the form.
...
2. ~~Web UI network bind + auth posture (loopback-default vs documented trusted-network).~~ **RESOLVED 2026-08-03 ([ADR 0019](../../docs/adr/0019-no-auth-on-the-config-ui-secrets-are-write-only.md)): no authentication**, trusted LAN behind Traefik, which is the only ingress — the container does not publish the port itself. The load-bearing half is the rule that comes with it: **secrets are write-only** — never rendered in a response, never in the enriched published state, never traced. A field holding a secret renders empty, and an empty submission means *unchanged*.
3. crates.io / Docker Hub release pipeline (later).
4. Log retention window (N days) — 24/7 tool must not fill the disk.
5. ~~**Broker/token secrets-at-rest boundary** when stored via the config file (same-file vs separate / env / `0600` / Docker secret)~~ **RESOLVED 2026-08-04 ([ADR 0023](../../docs/adr/0023-the-file-is-the-configuration-the-credential-stays-in-the-environment.md)): there is NO secrets-at-rest boundary, because no secret rests.** `config.toml` is the whole configuration; the smart-me credential (`SMARTME_CLIENT_ID` + `SMARTME_CLIENT_SECRET`, one pair, one source) stays in the environment and never descends to disk. This agrees with **line 204 below and with NFR12**, which have said exactly this since the architecture was written. *(Superseded: [ADR 0022](../../docs/adr/0022-secrets-rest-in-a-separate-0600-file.md) resolved this item on 2026-08-03 as a separate `0600` `secrets.toml`, contradicting both — and so made this document resolve its own item ⑤ two opposite ways, here and at line 204, for a day.)* **[#41](https://github.com/guycorbaz/smartme_mqtt/issues/41) no longer blocks Story 5.2**: with nothing confidential in `/data`, what remains is integrity — whoever can write the directory can replace `config.toml` and redirect the bridge — which is real, lesser, and nobody's prerequisite. `config.toml` is neither signed nor checksummed; directory permissions are the whole of the protection, stated rather than implied. The outbound half — never leaked into traced logs or the enriched published state — is [ADR 0019](../../docs/adr/0019-no-auth-on-the-config-ui-secrets-are-write-only.md). ~~*Coupled with item 2.*~~ **Item 2 is resolved, so this is unblocked rather than coupled — and it now carries MORE weight, not less**: with no login in front of the UI, a rendered secret is exposed to whoever can reach the host. [ADR 0019](../../docs/adr/0019-no-auth-on-the-config-ui-secrets-are-write-only.md) settles the *never rendered* rule; **where the value rests is still open and is the whole of this item.**
...
- **Startup validation:** config validated at startup; **refuse-to-publish on invalid** (FR26, amended by [ADR 0026](../../docs/adr/0026-a-configuration-it-cannot-use-stops-the-bridge-publishing-not-serving.md) — it was refuse-to-*start*, and exiting took down the screen that repairs the configuration) — table-driven, each invalid class → its *named* error. Fixture-first parsing contract (`fixtures/smartme_sample.json` + captured HTTP headers).
...
- **Secrets (item ⑤): `.env` / env vars only** (perms `0600`, never in the image, never logged incl. rotated logs). **The UI never reads, writes, or re-displays secrets** — only non-secret config. smart-me API key + optional broker password live in `.env`. *(Ratified 2026-08-04 by [ADR 0023](../../docs/adr/0023-the-file-is-the-configuration-the-credential-stays-in-the-environment.md), which restores this line and NFR12 after [ADR 0022](../../docs/adr/0022-secrets-rest-in-a-separate-0600-file.md) resolved item ⑤ the opposite way at line 121 without checking either. The credential is a **pair** — `client_id` travels with the secret, because a pair split across two sources can be rotated by halves and a mismatched pair presents as a smart-me outage rather than a configuration fault.)*
...
**Decision Compatibility:** All choices cohere. Rust edition-2024 + `tokio`/`axum`/`rumqttc`/`prost`/`reqwest` are mutually compatible at the verified versions. The 2-task model fits rumqttc's EventLoop reality; `ArcSwap<Config>` fits hot-reload-with-restart-only-for-structural-fields (FR46, 2026-08-03 — this line already said `ArcSwap` while line 73 said restart-only, and line 73 was the stale one); traced-drop + `try_publish` fit the non-blocking publish path; the freshness formula (`Date-header − ValueDate`) is consistent with the injected-`Clock` seam. No contradictory decisions found.
