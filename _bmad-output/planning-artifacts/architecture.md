...
5. ~~**Broker/token secrets-at-rest boundary** when stored via the config file (same-file vs separate / env / `0600` / Docker secret)~~ **RESOLVED 2026-08-03 ([ADR 0022](../../docs/adr/0022-secrets-rest-in-a-separate-0600-file.md)): a SEPARATE `secrets.toml` at `0600`**, beside a `0644` `config.toml` that stays diagnosable without privileges — so checking a topic never requires reading a file that holds a credential. The bridge **verifies the mode at startup and refuses to start** if it is group- or world-readable, because [#41](https://github.com/guycorbaz/smartme_mqtt/issues/41) already showed the displayed mode disagreeing with the enforced ACL on this very deployment. **Its prerequisite is NOT met: `/data` is world-writable, so Story 5.2 cannot close until #41 does.** Secrets are not encrypted at rest — permissions are the whole of the protection, stated rather than implied. The outbound half — never leaked into traced logs or the enriched published state — is [ADR 0019](../../docs/adr/0019-no-auth-on-the-config-ui-secrets-are-write-only.md). ~~*Coupled with item 2.*~~ **Item 2 is resolved, so this is unblocked rather than coupled — and it now carries MORE weight, not less**: with no login in front of the UI, a rendered secret is exposed to whoever can reach the host. [ADR 0019](../../docs/adr/0019-no-auth-on-the-config-ui-secrets-are-write-only.md) settles the *never rendered* rule; **where the value rests is still open and is the whole of this item.**
