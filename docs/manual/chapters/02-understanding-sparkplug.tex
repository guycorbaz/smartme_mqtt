...
\code{Node Control/Rebirth} & \textbf{absent} & \prog issues no MQTT subscription of any
                                kind, so no command can reach it. \\
...
published once, at QoS~0 and not retained --- so the broker keeps no copy. It holds no
subscription, so it never learns that a host has restarted and never re-births on its own.
Its own broker session is untouched by a host restarting, so nothing prompts it to
reconnect. And the protocol's remaining repair --- a rebirth request --- cannot reach it.

The consequence is that \textbf{after Ignition restarts, it may keep receiving data it has
no birth certificate to interpret}, with no mechanism in the protocol to restore the tag
definitions until \prog itself is restarted. A \emph{Sparkplug Aware} broker would close
this gap by replaying the stored births; Mosquitto is not one.

This is why the missing rebirth is the more urgent of the two gaps, and it is the reason
the \code{STATE} mechanism was measured on the live installation before being designed for.
\end{warning}
