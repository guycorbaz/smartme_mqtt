...
host. This is a real gap in \prog today; its practical consequences on this installation
are set out in section~\ref{sec:sp-where-we-sit} and chapter~\ref{ch:contract}.
\end{warning}
...
(section~\ref{sec:sp-rebirth}) is the only repair mechanism available.
\end{note}
...
\code{bdSeq} pairing & implemented & \textbf{Fixed per MQTT client}, not incremented per
                                CONNECT. A deliberate trade, explained in
...
Report by exception & \textbf{absent} & Publishes on a fixed interval. Recorded as a
                                deviation; it is blocked by the missing rebirth, because
                                periodic publishing currently substitutes for it. \\
\code{NCMD} subscription & implemented & \code{spBv1.0/<group>/NCMD/<node>} at QoS~1, on
                                every connect, before the birth. Commands are logged and
                                discarded --- see chapter~\ref{ch:contract}. \\
\code{Node Control/Rebirth} & \textbf{absent} & The request reaches \prog and is ignored:
                                nothing acts on any command yet. \\
Primary Host / \code{STATE} & \textbf{absent} & Not read, not waited for. \\
...
\begin{warning}
\textbf{The two missing mechanisms compound, and the cost is concrete.} \prog's NBIRTH is
published once, at QoS~0 and not retained --- so the broker keeps no copy. It reads no
...
it to reconnect. And the protocol's remaining repair --- a rebirth request --- reaches
\prog but is ignored.

The consequence is that \textbf{after Ignition restarts, it may keep receiving data it has
no birth certificate to interpret}. A \emph{Sparkplug Aware} broker would close this gap by
replaying the stored births; Mosquitto is not one.

\textbf{What does restore the definitions is any reconnect on \prog's own side}, since a
fresh NBIRTH is published on every successful MQTT connection --- a broker restart, a
network interruption or a keep-alive expiry all repair the consumer's view. What no event on
the \emph{host} side can do is prompt that reconnect. (An earlier edition of this paragraph
...
This is why the missing rebirth handling is the more urgent of the two gaps --- more so now
than when this was written, because the subscription exists and only the handler is
missing --- and it is the reason the \code{STATE} mechanism was measured on the live
installation before being designed for.
\end{warning}
