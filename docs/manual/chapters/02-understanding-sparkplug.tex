...
specification itself, kept in this repository (see
...
\midrule
Topic namespace, device level & implemented & One node, one device per meter, named by the
                                meter's short name (contract~13; its serial until then). \\
NBIRTH / DBIRTH / DDATA / NDEATH & implemented & \code{NDATA} and \code{DDEATH} are not
                                emitted --- see chapter~\ref{ch:contract}. \\
Will registered inside CONNECT & implemented & At \textbf{QoS~1}, as the specification
                                requires. Corrected 2026-08-10. \\
...
The Sparkplug~B specification is kept in this repository, at

\begin{center}\code{docs/spec/sparkplug-b-3.0.0/}\end{center}

\noindent release tag v3.0.0, EPL-2.0. Keeping a copy means it can be grepped and cited
...
worst defects found so far came from trusting a secondary source --- a supplier
