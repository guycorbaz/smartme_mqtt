...
\textbf{retain false}, which is what the Sparkplug specification requires for those
messages. Retain is the important half: a retained payload is a value the broker replays
to any future subscriber with no way to judge its age --- a stored lie that outlives the
process. Freshness is the BIRTH's job, not the broker's.

\begin{note}
\textbf{The registered will is the exception, and \prog gets it wrong today.} The
...
else. In practice a broker holding a will delivers it once regardless, so the observable
effect is small; it is nonetheless a violation and it is listed under
...
\begin{itemize}
  \item \textbf{No NCMD subscription}, so \code{Node Control/Rebirth} cannot be honoured.
        ...
        own. \prog issues no MQTT subscription of any kind.
  ...
  \item \textbf{No \emph{Primary Host Application} support.} Sparkplug lets an Edge Node
        ...
        not any consumer is listening, and never reads a \code{STATE} topic. On a
        single-broker installation with a permanently connected host this is invisible;
        it matters in a redundant-broker topology, where an Edge Node can otherwise be
        stranded on a broker its host cannot reach.
