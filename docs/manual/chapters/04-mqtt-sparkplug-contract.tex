...
\begin{warning}
Contract version~1 published \code{500} for \textsc{stale} and \code{0} for \textsc{bad}.
Both were wrong, and wrong in the dangerous direction: a real host displayed \code{500} as
\emph{Good(500)} — a good-\emph{level} code with an unrecognised subcode — and \code{0} is
that host's own code for \emph{good, unspecified}. A version-1 bridge therefore told its
SCADA that an unusable value was trustworthy. Found by the Tier-3 contract test and fixed
in contract version~2. Nothing inside the bridge could have detected it, which is precisely
