...
\begin{warning}
\textbf{There is one state with no web interface at all: a \code{config.toml} that is present
and invalid \emph{at startup}.} The bridge refuses to start, and refusing means no screen ---
so the repair is to fix the file on the host, not in a browser. The faults are printed on
\code{stderr} and in \code{docker compose logs}, each naming the key to edit.

This matters most after an image update that changes \code{schema\_version}: the previous
version's file is refused, and under a restart policy the container will loop. If instead the
file becomes unusable \emph{while the bridge is already running}, it stays up and says so ---
the screens remain available, because an operator is at the keyboard and the screen is the
repair tool.
\end{warning}
