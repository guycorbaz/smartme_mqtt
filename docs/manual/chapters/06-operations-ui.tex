...
A \emph{fault} is not a deliberate silence, and the two are reported apart. When the smart-me
cloud refuses or cannot answer for a meter, that meter's name appears on \code{/} --- ``one
meter is not being read: \emph{cellar}'' --- and in \code{/healthz} under
\code{failed\_sources}, which is an empty list when everything is being read.

The status code stays \code{200}, and that is deliberate rather than lenient: the fault a
refused credential represents is not one a restart clears, so returning \code{503} would put
the container in a loop and destroy the screen that names the problem. What a restart
\emph{does} fix --- a poll loop that has stopped looping --- is what \code{503} is reserved
for.

Until 2026-08-07 the page said the bridge was ``polling the meters and publishing what it
reads'' whatever the meters were doing, because it had no way to see them. The last known
values are still published while a meter is unreadable, marked not-good, so nothing
downstream shows them as current.
