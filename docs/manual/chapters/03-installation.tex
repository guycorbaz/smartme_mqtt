...
A slow smart-me server cannot produce the \code{503}. One fetch is abandoned after
ten seconds, and the loop is only called wedged once it has missed three publish
periods --- the shortest of which is five seconds. That margin is held by a test
rather than by arithmetic nobody re-does.
