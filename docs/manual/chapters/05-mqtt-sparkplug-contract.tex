...
\begin{itemize}
  ...
  \item \textbf{No \emph{Primary Host Application} support.} Sparkplug lets an Edge Node
        ...
        asks. Ignition's MQTT Engine does ask, on its own, when it receives data from a
        node whose birth it never saw --- which is precisely this situation. But \prog
        still reads no \code{STATE} topic, so it never learns the host came back, and it
        will not volunteer a birth. A host that does \emph{not} ask still sees
