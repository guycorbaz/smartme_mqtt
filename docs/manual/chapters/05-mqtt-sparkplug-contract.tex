...
\begin{itemize}
  ...
  \item \textbf{No \emph{Primary Host Application} support.} Sparkplug lets an Edge Node
        ...
        retained, so the broker keeps no copy for a returning consumer; \prog holds no
        subscription, so it never learns the host came back and never re-births; and its
        own broker session is untouched by the host restarting, so nothing prompts it to
        reconnect. The protocol's remaining repair is an \code{NCMD}
        \code{Node Control/Rebirth}, which \prog also does not implement. The consequence
        is that \textbf{after a consumer restarts, it may keep receiving \code{NDATA} it
        has no \code{NBIRTH} to interpret, and no mechanism in the protocol restores the
        tag definitions} until \prog itself is restarted.
