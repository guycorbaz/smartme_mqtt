...
\begin{itemize}
  ...
  \item \textbf{No \emph{Primary Host Application} support.} Sparkplug lets an Edge Node
        ...
        subscription, so it never learns the host came back; and its own broker session is
        untouched by the host restarting, so \emph{that} event prompts nothing. The
        protocol's own repair for this is an \code{NCMD} \code{Node Control/Rebirth},
        which \prog does not implement. The consequence is that \textbf{after a consumer
