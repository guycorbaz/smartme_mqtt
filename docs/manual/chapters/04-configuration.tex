...
\begin{warning}
\textbf{This is the one error validation cannot catch.} \prog refuses a serial that cannot be a
topic level, and two meters that would collide on one topic. It cannot tell that \code{garage} is
pointed at the cellar's device: that configuration is well-formed, unique, complete --- and wrong.
Only you can see it, and only by reading the serial beside the topic.
