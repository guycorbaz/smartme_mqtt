...
\code{false}. Changing something that is not the mapping --- the publish period, the broker, the log
settings --- does not, because it costs you a click for a change you can already see is unrelated,
and a guard that fires for no visible reason is a guard that gets switched off.
...
present and invalid makes \prog refuse to start}, listing every fault at once
(chapter~\ref{ch:trouble}). Absence is not invalidity: a bridge that refused to
start without a configuration could never be configured at all, because the screen that
writes the first one would sit behind a process that had already exited.
