...
\textbf{\code{config.toml} is the configuration.} It lives in the state directory, it is
edited from the web UI, and it holds every setting \prog has. The environment carries
exactly two things and no others: the smart-me credential, which never descends to disk,
and \code{SMARTME\_STATE\_DIR}, because a file cannot say where it lives.
...
settings in \code{config.toml}, changed from the web UI or written by hand:
