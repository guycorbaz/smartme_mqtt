...
\begin{warning}
\textbf{As of this build, \code{config.toml} is written BY HAND. There is no editing screen
yet.} The web UI exists and answers --- it tells you which state the bridge is in and serves
\code{/healthz} --- but the forms that write this file are specified and not built
(story~6.2).

This matters more than it sounds, because of \code{mapping\_confirmed}
(section~\ref{sec:confirm}): a configuration that is valid but unconfirmed publishes
\emph{nothing at all}. So a first run done by hand must set
\code{mapping\_confirmed = true} in the file, or the bridge will come up, serve its page,
and stay silent for ever while looking healthy.

This warning is removed by story~6.2, not before.
\end{warning}
...
directory, set the credential in \code{.env}, and start the container.
