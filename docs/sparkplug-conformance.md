...
| `topics-dcmd-topic` | MUST | **not implemented** — no DCMD subscription, and a subscriber must build this topic form too. **⏳ TIME-LIMITED, recorded at Story 4.7:** a planned meter relay command is a writable Device output, which is the condition its chapter-5 twin `-device-dcmd-subscribe` is `n/a` upon; if it ever lands, both go live together. Not re-verdicted here — the reminder now lives on `MessageType` in `sparkplug-b/src/topic.rs` — the one edit no DCMD implementation can avoid — after [#38](https://github.com/guycorbaz/smartme_mqtt/issues/38) was closed on 2026-08-28 with the relay not planned *"for the time being"* | — | **gap (unimplemented)** (**Story 4.19**, re-owned) — Story 4.6 declined DCMD deliberately: `-device-dcmd-subscribe` is conditional on *"if the Device supports writing to outputs"* (`:403-407`) and no device here does. `MessageType` has no `DCmd` variant on purpose. See the criterion note below: this row probably belongs at `n/a` |
...
| `topics-ddata-seq-num` | MUST | one shared edge-node counter, incremented per message and wrapping at 255 | `device_messages_share_the_edge_node_numbering`, `prop_seq_stays_in_range_and_wraps_at_the_boundary`, and `sequence_numbering_is_continuous_across_node_and_device_messages` | conformant |
...
| `message-flow-device-birth-publish-dbirth-topic` | MUST | `spBv1.0/{group}/DBIRTH/{node}/{device}` | `device_topics_append_the_device_identifier`; the literal `spBv1.0/Site/DBIRTH/Bridge/garage` is asserted — **the device level is the meter's published name from contract v13** (ADR 0049), and the serial travels as the `serial` property of the DBIRTH's metrics | conformant |
...
| `case-sensitivity-metric-names` | SHOULD NOT | the metric names are six constants — `Power`, `Energy`, `Cause/Power`, `Cause/Energy`, `Contract/Version` and `Node Control/Rebirth` — whose lower-cased forms are all distinct. *(Re-checked at contract v13 rather than carried over. Two of the six — the cause metrics — are new since this row was last verdicted at Story 4.7, and all four measurement names were renamed and un-renamed on 2026-08-25 (ADR 0050), which is exactly the kind of movement that leaves a stale verdict behind. Lower-cased they read `power`, `energy`, `cause/power`, `cause/energy`, `contract/version`, `node control/rebirth` — six distinct strings, no pair differing only in case. An unchecked verdict on a live SHOULD NOT is the thing this column exists to prevent.)* | — **mutation-tested: renaming `Energy` to `POWER`, so the DBIRTH carries `Power` and `POWER`, leaves all 69 tests green** | **gap (unproven)** ([#30](https://github.com/guycorbaz/smartme_mqtt/issues/30)) |
...
the reminder now lives on `MessageType` in `sparkplug-b/src/topic.rs` — the one edit no DCMD implementation can avoid — after [#38](https://github.com/guycorbaz/smartme_mqtt/issues/38) was closed on 2026-08-28 with the relay not planned *"for the time being"*. An `n/a` whose condition
