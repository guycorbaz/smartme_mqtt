...
// `persist_atomic_with_mode` lived here until 2026-08-04. It created a file
// `0600` from the first byte rather than tightening it afterwards, for
// ADR 0022's `secrets.toml`. ADR 0023 removed the secret from disk entirely, so
// there is nothing left to open restrictively — and the reasoning it existed for
// (the umask applies at `File::create`, and the content lands in the TEMP file,
// so a later `chmod` of the final path would not even cover the window) is
// recorded in ADR 0022 for whoever needs it next.
